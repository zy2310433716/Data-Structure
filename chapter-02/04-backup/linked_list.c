...
{
    Node *p = l->next;                // 从首元结点开始（跳过头结点）
    while (p != NULL && p->data != e) // 一直往后找，直到值相等或到结尾
    {
        p = p->next;
    }
    return p; // 找到则返回该节点，找不到返回NULL
}
...
int main()
{
    ...
    // 按位查找
    int pos = 2;
    printf("找到了第 %d 个位置的值为%d\n", pos, findElem(list, pos));

    // 按值查找
    ElemType flag = 740;
    printf("找到了%d 元素的指针为%p\n", flag, locateElem(list, 740));
